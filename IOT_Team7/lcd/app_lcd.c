...
void memlcd_update(global_data_t *data)
{
  ...
  snprintf(tempStr, 50, "Temp: %d.%d *C", data->tempInt, data->tempFrac);
  snprintf(humidStr, 50, "Humid: %d.%d %%", data->humidInt, data->humidFrac);
  snprintf(rPeriodStr, 50, "Sample Period: %d", data->rPeriod);
  snprintf(advPeriodStr, 50, "ADV BLE Time: %d", data->advPeriod);
  snprintf(cntStr, 50, "Counter: %d", data->cnt);

  GLIB_clear(&glibContext);

   GLIB_drawStringOnLine(&glibContext,
                         "TEAM 7",
                         line++,                 // Tăng dòng sau khi vẽ
                         GLIB_ALIGN_CENTER,      // Căn giữa
                         0,                      // Không cần offset ngang
                         5,                      // Không cần offset dọc
                         true);
line++;
   GLIB_drawStringOnLine(&glibContext,
                         tempStr,
                         line++, // Vẽ ở dòng tiếp theo
                         GLIB_ALIGN_LEFT,
                         5,
                         5,
                         true);
